...
class BNSDatasetRegression(BNSBaseDataset):
    def __getitem__(self, idx):
        ...
        X_sequence = torch.as_tensor(seq, dtype=str_to_dtype(self.cfg.strain_precision))  # TODO: fix

        y_target = self._get_vars(f, self.cfg.target_variables, idx, dtype=str_to_dtype(self.cfg.variables_precision)) # TODO
        z_observed = self._get_vars(f, self.cfg.observed_variables, idx, dtype=str_to_dtype(self.cfg.variables_precision)) # TODO
