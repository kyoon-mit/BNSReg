...
from BNSReg.dataset.regression_l1_dataset import BNSDatasetRegressionL1

class LitBNSDataRegressionL1(L.LightningDataModule):
    ...
    def setup(self, stage: str | None = None):
        ...
        if stage == "fit":
            ...
            self.train_dataset = BNSDatasetRegressionL1('train', self.cfg)
            self.val_dataset = BNSDatasetRegressionL1('val', self.cfg)

        # Assign test dataset for use in dataloader
        elif stage in ("test", "predict"):
            self.test_dataset = BNSDatasetRegressionL1('test', self.cfg)
