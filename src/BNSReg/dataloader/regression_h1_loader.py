...
from BNSReg.dataset.regression_h1_dataset import BNSDatasetRegressionH1

class LitBNSDataRegressionH1(L.LightningDataModule):
    ...
    def setup(self, stage: str | None = None):
        ...
        if stage == "fit":
            ...
            self.train_dataset = BNSDatasetRegressionH1('train', self.cfg)
            self.val_dataset = BNSDatasetRegressionH1('val', self.cfg)

        # Assign test dataset for use in dataloader
        elif stage in ("test", "predict"):
            self.test_dataset = BNSDatasetRegressionH1('test', self.cfg)
